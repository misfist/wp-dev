var app = angular.module('app', ['ngRoute', 'ngSanitize', 'slick', 'app.controllers', 'app.directives']);
...
//Content controller
app.controller('Content', ['$scope', 'WPService', '$routeParams', '$http', function($scope, WPService, $routeParams, $http) {
	$http.get(WPService.jsonUrl + 'posts/' + $routeParams.ID).success(function(res){
		$scope.post = res;
	...
	$http.get(WPService.jsonUrl + 'media?filter[post_parent]=' + $routeParams.ID + '&filter[posts_per_page]=-1').success(function(res){
		if ( res.length > 1 ) {
			$scope.media = res;
		}
	});
...
app.controller('Category', ['$scope', '$routeParams', '$http', 'WPService', function($scope, $routeParams, $http, WPService) {
	WPService.getAllCategories();
	$http.get(WPService.jsonUrl + 'terms/category/?filter[slug]=' + $routeParams.category).success(function(res){
		if (!res.length) {
