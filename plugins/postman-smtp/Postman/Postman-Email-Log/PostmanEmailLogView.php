...
class PostmanEmailLogView extends WP_List_Table {
	...
	function column_title($item) {
		...
		if (! (empty ( $meta_values ['original_to'] [0] ) && empty ( $meta_values ['originalHeaders'] [0] ))) {
			...
			$actions ['resend'] = sprintf ( '<span id="%3$s"><a href="javascript:postman_resend_email(%1$s);">%2$s</a></span>', $item ['ID'], _x ( 'Resend', 'Attempt to resend this email', 'postman-smtp' ), 'resend-' . $item ['ID'] );
		} else {
