...
class PostmanEmailLogController {
    ...
    background: #bbb;
			...
			if (! empty ( $meta_values ['reply_to_header'] [0] )) {
				printf ( '<tr><th style="text-align:right">%s:</th><td>%s</td></tr>', _x ( 'Reply-To', 'The return email address', 'postman-smtp' ), esc_html ( $meta_values ['reply_to_header'] [0] ) );
			}
    ...
    background: #bbb;
			...
			printf ( '<p>%s</p>', __ ( 'This is the conversation between Postman and your mail server. It can be useful for diagnosing problems. <b>DO NOT</b> post it on-line, it may contain your account password.', 'postman-smtp' ) );
			print '<hr/>';
			print '<pre>';
	...
	function postman_email_log_enqueue_resources() {
		...
		wp_localize_script ( 'postman_resend_email_script', 'postman_js_email_was_resent', __ ( 'Email was successfully resent (but without attachments)', 'postman-smtp' ) );
		wp_localize_script ( 'postman_resend_email_script', 'postman_js_email_not_resent', __ ( 'Email could not be resent. Error: %s', 'postman-smtp' ) );
		wp_localize_script ( 'postman_resend_email_script', 'postman_js_resend_label', __ ( 'Resend', 'postman-smtp' ) );
	}
...
<div class="wrap">
	...
	<div
		...
		echo __ ( 'This is a record of deliveries made to the mail server. It does not neccessarily indicate sucessful delivery to the recipient.', 'postman-smtp' )?></p>
	</div>
