...
class PostmanSendGridTransport extends PostmanAbstractModuleTransport implements PostmanModuleTransport {
	...
	public function getName() {
		return _x ( 'SendGrid API', 'Transport Name', 'postman-smtp' );
	}
	/**
	 * v0.2.1
	 *
	...
	public function getDeliveryDetails() {
		...
		return sprintf ( __ ( 'Postman will send mail via the <b>%1$s %2$s</b>.', 'postman-smtp' ), '🔐', $this->getName () );
	}
	...
	protected function validateTransportConfiguration() {
		...
		$apiKey = $this->options->getSendGridApiKey ();
		if (empty ( $apiKey )) {
			array_push ( $messages, __ ( 'API Key can not be empty.', 'postman-smtp' ) );
			$this->setNotConfiguredAndReady ();
		}
		if (! $this->isSenderConfigured ()) {
			array_push ( $messages, __ ( 'Message From Address can not be empty.', 'postman-smtp' ) );
			$this->setNotConfiguredAndReady ();
		}
		return $messages;
	}
	...
	public function addSettings() {
		...
		add_settings_section ( PostmanSendGridTransport::SENDGRID_AUTH_SECTION, __ ( 'Authentication', 'postman-smtp' ), array (
				$this,
				'printSendGridAuthSectionInfo' 
		), PostmanSendGridTransport::SENDGRID_AUTH_OPTIONS );
		
		add_settings_field ( PostmanOptions::SENDGRID_API_KEY, __ ( 'API Key', 'postman-smtp' ), array (
				$this,
	...
	public function printSendGridAuthSectionInfo() {
		printf ( '<p id="wizard_sendgrid_auth_help">%s</p>', sprintf ( __ ( 'Create an account at <a href="%s" target="_new">SendGrid.com</a> and enter <a href="%s" target="_new">an API key</a> (<em>not</em> an API Key ID) below.', 'postman-smtp' ), 'https://sendgrid.com', 'https://app.sendgrid.com/settings/api_keys' ) );
	}
	
	/**
	 */
	public function sendgrid_api_key_callback() {
		printf ( '<input type="password" autocomplete="off" id="sendgrid_api_key" name="postman_options[sendgrid_api_key]" value="%s" size="60" class="required" placeholder="%s"/>', null !== $this->options->getSendGridApiKey () ? esc_attr ( PostmanUtils::obfuscatePassword ( $this->options->getSendGridApiKey () ) ) : '', __ ( 'Required', 'postman-smtp' ) );
	...
	public function printWizardAuthenticationStep() {
		...
		printf ( '<label for="api_key">%s</label>', __ ( 'API Key', 'postman-smtp' ) );
		print '<br />';
