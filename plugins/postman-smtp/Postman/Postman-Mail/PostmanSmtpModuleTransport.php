...
class PostmanSmtpModuleTransport extends PostmanAbstractZendModuleTransport implements PostmanZendModuleTransport {
	...
	protected function validateTransportConfiguration() {
		...
		if (! $this->isHostConfigured ( $this->options )) {
			array_push ( $messages, __ ( 'Outgoing Mail Server Hostname and Port can not be empty.', 'postman-smtp' ) );
			$this->setNotConfiguredAndReady ();
		}
		if (! $this->isEnvelopeFromConfigured ()) {
			array_push ( $messages, __ ( 'Envelope Email Address can not be empty.', 'postman-smtp' ) );
			$this->setNotConfiguredAndReady ();
		}
		if ($this->options->isAuthTypePassword () && ! $this->isPasswordAuthenticationConfigured ( $this->options )) {
			array_push ( $messages, __ ( 'Username and password can not be empty.', 'postman-smtp' ) );
			$this->setNotConfiguredAndReady ();
		}
		if ($this->getAuthenticationType () == PostmanOptions::AUTHENTICATION_TYPE_OAUTH2) {
			if (! $this->isOAuth2SupportedHostConfigured ()) {
				...
				array_push ( $messages, sprintf ( __ ( 'OAuth 2.0 authentication requires a supported OAuth 2.0-capable Outgoing Mail Server.', 'postman-smtp' ) ) );
				$this->setNotConfiguredAndReady ();
			}
		}
		if (empty ( $messages )) {
			...
			if ($this->isPermissionNeeded ( $this->options, $this->getOAuthToken () )) {
				...
				$message = sprintf ( __ ( 'You have configured OAuth 2.0 authentication, but have not received permission to use it.', 'postman-smtp' ), $this->getScribe ()->getClientIdLabel (), $this->getScribe ()->getClientSecretLabel () );
	...
	public function getConfigurationBid(PostmanWizardSocket $hostData, $userAuthOverride, $originalSmtpServer) {
		...
		if ($score > 10) {
			...
			$recommendation ['message'] = sprintf ( __ ( 'Postman recommends %1$s with %2$s authentication to host %4$s on port %3$d.', 'postman-smtp' ), $transportDescription, $authDesc, $port, $hostname );
		}
	...
	public function addSettings() {
		...
		// Sanitize
		add_settings_section ( PostmanAdminController::SMTP_SECTION, __ ( 'Transport Settings', 'postman-smtp' ), array (
				$this,
		...
		add_settings_field ( PostmanOptions::ENVELOPE_SENDER, __ ( 'Envelope Email Address', 'postman-smtp' ), array (
				$this,
				'sender_email_callback' 
		), PostmanAdminController::SMTP_OPTIONS, PostmanAdminController::SMTP_SECTION );
		
		add_settings_field ( PostmanOptions::SECURITY_TYPE, _x ( 'Security', 'Configuration Input Field', 'postman-smtp' ), array (
				$this,
				'encryption_type_callback' 
		), PostmanAdminController::SMTP_OPTIONS, PostmanAdminController::SMTP_SECTION );
		
		add_settings_field ( PostmanOptions::AUTHENTICATION_TYPE, __ ( 'Authentication', 'postman-smtp' ), array (
				$this,
				'authentication_type_callback' 
		), PostmanAdminController::SMTP_OPTIONS, PostmanAdminController::SMTP_SECTION );
		
		add_settings_section ( PostmanAdminController::BASIC_AUTH_SECTION, __ ( 'Authentication', 'postman-smtp' ), array (
				$this,
				'printBasicAuthSectionInfo' 
		), PostmanAdminController::BASIC_AUTH_OPTIONS );
		
		add_settings_field ( PostmanOptions::BASIC_AUTH_USERNAME, __ ( 'Username', 'postman-smtp' ), array (
				$this,
		...
		// the OAuth section
		add_settings_section ( PostmanAdminController::OAUTH_SECTION, __ ( 'Authentication', 'postman-smtp' ), array (
				$this,
	...
	public function hostname_callback() {
		printf ( '<input type="text" id="input_hostname" name="postman_options[hostname]" value="%s" size="40" class="required" placeholder="%s"/>', null !== $this->options->getHostname () ? esc_attr ( $this->options->getHostname () ) : '', __ ( 'Required', 'postman-smtp' ) );
	}
	...
	public function encryption_type_callback() {
		...
		printf ( '<option class="input_enc_type_none" value="%s" %s>%s</option>', PostmanOptions::SECURITY_TYPE_NONE, $encType == PostmanOptions::SECURITY_TYPE_NONE ? 'selected="selected"' : '', __ ( 'None', 'postman-smtp' ) );
	...
	public function printBasicAuthSectionInfo() {
		print __ ( 'Enter the account credentials.', 'postman-smtp' );
	}
	...
	public function basic_auth_username_callback() {
		...
		$inputDescription = __('The Username is usually the same as the Envelope Email Address.','postman-smtp');
		...
		printf ( '<input type="text" id="input_basic_auth_username" name="postman_options[basic_auth_username]" value="%s" size="40" class="required" placeholder="%s"/><br/><span class="postman_input_description">%s</span>', $inputValue, __ ( 'Required', 'postman-smtp' ), $inputDescription );
	}
	...
	public function basic_auth_password_callback() {
		...
		printf ( '<input type="password" id="input_basic_auth_password" name="postman_options[basic_auth_password]" value="%s" size="40" class="required" placeholder="%s"/>', null !== $this->options->getPassword () ? esc_attr ( PostmanUtils::obfuscatePassword ( $this->options->getPassword () ) ) : '', __ ( 'Required', 'postman-smtp' ) );
	...
	public function oauth_client_id_callback() {
		printf ( '<input type="text" onClick="this.setSelectionRange(0, this.value.length)" id="oauth_client_id" name="postman_options[oauth_client_id]" value="%s" size="60" class="required" placeholder="%s"/>', null !== $this->options->getClientId () ? esc_attr ( $this->options->getClientId () ) : '', __ ( 'Required', 'postman-smtp' ) );
	}
	...
	public function oauth_client_secret_callback() {
		printf ( '<input type="text" onClick="this.setSelectionRange(0, this.value.length)" autocomplete="off" id="oauth_client_secret" name="postman_options[oauth_client_secret]" value="%s" size="60" class="required" placeholder="%s"/>', null !== $this->options->getClientSecret () ? esc_attr ( $this->options->getClientSecret () ) : '', __ ( 'Required', 'postman-smtp' ) );
	}
	...
	public function sender_email_callback() {
		...
		$requiredLabel = __ ( 'Required', 'postman-smtp' );
		$envelopeFromMessage = __ ( 'The Envelope Email Address, like the return address printed on an envelope, identifies the account owner to the SMTP server.', 'postman-smtp' );
		$spfMessage = sprintf ( __ ( 'For reliable delivery, this domain must specify an <a target="_new" href="%s">SPF record</a> permitting the use of the SMTP server named above.', 'postman-smtp' ), 'https://www.mail-tester.com/spf/' );
	...
	public function printWizardMailServerHostnameStep() {
		...
		printf ( '<p id="godaddy_spf_required"><span style="background-color:yellow">%s</span></p>', sprintf ( __ ( '<b>Warning</b>: If you own this domain, make sure it has an <a href="%s">SPF record authorizing GoDaddy</a> as a relay, or you will have delivery problems. If you have access to cPanel, enable the Remote Mail Exchanger.', 'postman-smtp' ), 'http://www.mail-tester.com/spf/godaddy' ) );
	}
	...
	public function printWizardAuthenticationStep() {
		...
		printf ( '<label for="username">%s</label>', __ ( 'Username', 'postman-smtp' ) );
		print '<br />';
