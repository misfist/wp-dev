...
class PostmanMandrillTransport extends PostmanAbstractModuleTransport implements PostmanModuleTransport {
	...
	public function getName() {
		return _x ( 'Mandrill API', 'Transport Name', 'postman-smtp' );
	}
	/**
	 * v0.2.1
	 *
	...
	public function addSettings() {
		...
		add_settings_section ( PostmanMandrillTransport::MANDRILL_AUTH_SECTION, __ ( 'Authentication', 'postman-smtp' ), array (
				$this,
				'printMandrillAuthSectionInfo' 
		), PostmanMandrillTransport::MANDRILL_AUTH_OPTIONS );
		
		add_settings_field ( PostmanOptions::MANDRILL_API_KEY, __ ( 'API Key', 'postman-smtp' ), array (
				$this,
	...
	public function printMandrillAuthSectionInfo() {
		printf ( '<p id="wizard_mandrill_auth_help">%s</p>', sprintf ( __ ( 'Create an account at <a href="%s" target="_new">Mandrillapp.com</a> and enter <a href="%s" target="_new">an API key</a> below.', 'postman-smtp' ), 'https://mandrillapp.com', 'https://mandrillapp.com/settings' ) );
	}
	
	/**
	 */
	public function mandrill_api_key_callback() {
		printf ( '<input type="password" autocomplete="off" id="mandrill_api_key" name="postman_options[mandrill_api_key]" value="%s" size="60" class="required" placeholder="%s"/>', null !== $this->options->getMandrillApiKey () ? esc_attr ( PostmanUtils::obfuscatePassword ( $this->options->getMandrillApiKey () ) ) : '', __ ( 'Required', 'postman-smtp' ) );
