	...
	public function getReadyMessage() {
		if ($this->getCurrentTransport ()->isConfiguredAndReady ()) {
			if (PostmanOptions::getInstance ()->getRunMode () != PostmanOptions::RUN_MODE_PRODUCTION) {
				return __ ( 'Postman is in <em>non-Production</em> mode and is dumping all emails.', 'postman-smtp' );
			} else {
				return __ ( 'Postman is configured.', 'postman-smtp' );
			}
		} else {
			return __ ( 'Postman is <em>not</em> configured and is mimicking out-of-the-box WordPress email delivery.', 'postman-smtp' );
		}
	}
}
